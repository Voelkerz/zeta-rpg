...
namespace ZetaGames.RPG {
    public class NpcBuildGoal {
        ...
        public void CreateShelterBuildGoal() {
            if (npc.stats.homeProperty != null) {
                // build a campsite
            } else {
                // no home, need to build one
                planned = true;
                structCatGoal = StructureCategory.Home;
                structTypeGoal = StructureType.Small_House;
                structQualityGoal = EconomicClass.Poor;
        ...
        private BaseStructureData GetStructureData() {
            foreach (BaseStructureData data in npc.buildingManager.buildableStructures) {
        ...
        private void PlaceBuildSite() {
            ...
            buildingTileList.AddRange(structureData.blockedTiles);
            buildingTileList.AddRange(structureData.walkableTiles);
            buildingTileList.AddRange(structureData.wallBoundary);
            buildingTileList.Add(structureData.doorTile);
